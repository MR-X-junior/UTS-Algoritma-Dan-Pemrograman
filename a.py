...
matkul = [["Pendidikan Pancasila", 1],
          ...
          ["Multimedia dan Desain Grafis", 1],
    ...
    while True:
      ...
      if any(not p.isdigit() or int(p) < 1 or int(p) > len(matkul) for p in pilih):
          ...
          if total_sks < 20:
              ...
              if ganti:
                  tersedia = [i + 1 for i in range(len(matkul)) if str(i + 1) not in pilih]
                  print(f"[+] Maksimal {len(tersedia)} mata kuliah yang bisa diganti")
                  while True:
                      choice = input(f"[?] Pilih mata kuliah yang ingin diganti (1-{len(pilih)}): ").strip().split(",")
                      if any(not c.isdigit() or int(c) < 1 or int(c) > len(pilih) for c in choice):
                          print("[✗] Pilihan tidak valid. Silakan pilih nomor mata kuliah yang sesuai.\n")
                      elif len(choice) == 0:
                          print("[✗] Anda harus memilih setidaknya satu mata kuliah untuk diganti.\n")
                      elif len(choice) > len(tersedia):
                          print("[✗] Jumlah mata kuliah yang ingin diganti melebihi jumlah mata kuliah yang tersedia.\n")
                      else:

                          print("\n[+] Mata kuliah yang tersedia untuk diganti:\n")
                          for i in range(len(tersedia)):
                              print(f"[{i + 1:02d}] {matkul[tersedia[i] - 1][0]} ({matkul[tersedia[i] - 1][1]} SKS)")

                          while True:
                              ganti = input(f"\n[?] Pilih mata kuliah pengganti (1-{len(tersedia)}): ").strip().split(",")
                              if any(not g.isdigit() or int(g) < 1 or int(g) > len(tersedia) for g in ganti):
                                  print("[✗] Pilihan tidak valid. Silakan pilih nomor mata kuliah yang sesuai.\n")
                              elif len(ganti) != len(choice):
                                  print("[✗] Jumlah mata kuliah pengganti harus sama dengan jumlah mata kuliah yang ingin diganti.\n")
                              else:
                                  for i in range(len(choice)):
                                      pilih[int(choice[i]) - 1] = str(tersedia[int(ganti[i]) - 1])

                                  total_sks = 0

                                  for i in ganti:
                                      total_sks += matkul[((tersedia[int(i) - 1]) - 1)][1]
                                  
                                  for i in range(len(pilih)):
                                      if str(i) not in choice:
                                          total_sks += matkul[int(pilih[i]) - 1][1]
                                  ...
                                  if total_sks < 20:
                                      ...
                                      print("\n[+] Mata kuliah yang dipilih setelah diganti:\n")
                                      for i in range(len(pilih)):
                                           print(f"[{i + 1:02d}] {matkul[int(pilih[i]) - 1][0]} ({matkul[int(pilih[i]) - 1][1]} SKS)")
                                      print(f"\n[+] Total SKS diambil: {total_sks} / 20")
                                      break
